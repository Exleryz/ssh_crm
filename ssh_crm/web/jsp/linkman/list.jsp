                    ...
                    <TD height=25>
                        <FORM id="pageForm" name="customerForm"
                            ...
                            <input type="hidden" name="pageSize" id="pageSizeInput"
                                   value="<s:property value="#pageBean.pageSize"></s:property>"/>
                            <TABLE cellSpacing=0 cellPadding=2 border=0>
                                ...
                                <TR>
                                    ...
                                    <td>客户名称: </td>
                                    <td>
                                        <input type="hidden" name="customer.cust_id" id="cust_id" value="${param['customer.cust_name']}"/>
                                        <input  class=textbox id="cust_name"
                                                style="WIDTH: 80px" maxLength=50 name="cust_name" id="${param['cust_name']}"/>
                                        <input type="button" value="选择客户"
                                               onclick="window.open('${pageContext.request.contextPath}/CustomerAction_list?select=true', '', 'width=600,height=300')"/></td>
...
共[<B><s:property value="#pageBean.totalCount"></s:property></B>]条记录,[<B><s:property
        value="#pageBean.totalPage"></s:property></B>]页
,每页显示
<select name="pageSize" onchange="changePageSize($('#pageSizeSelect option').filter(':selected').val())"
        id="pageSizeSelect">
...
<option value="5" <s:property value="#pageBean.pageSize==5? 'selected':''"/>>5</option>
    ...
    [<A href="javaScript:void(0)"
        onclick="changePage(<s:property value='#pageBean.currentPage - 1'></s:property>)">前一页</A>]
    <B><s:property value="#pageBean.currentPage"></s:property></B>
    [<A href="javascript:void(0)"
        onclick="changePage(<s:property value='#pageBean.currentPage + 1'></s:property>)">后一页</A>]
    到
